...
function generateIntel(): RoundIntel[] {
  ...
  const r1Qua: IntelCard[] = slot('QUA', [
    ...
    '教研投入本学期即时竞争力加成高达+10，是所有策略中即时效果最强的，远超奖学金抢生。',
    '据内部消息教研升级成本本学期有优惠补贴仅需¥3万，且教学质量直接+25，一步到位建立壁垒。',
  ], sources, 1, 1)
  ...
  const r1Mkt: IntelCard[] = slot('MKT', [
    ...
    '口碑宣传每次可提升声望+40，只需两学期即可突破溢价变现门槛，回报速度极快。',
  ...
  const r1Hold: IntelCard[] = slot('HOLD', [
    ...
    '稳健办学利润率高达50%，且完全不受其他学校抢生行为的竞争力惩罚影响。',
    '连续稳健办学可获得教育局额外补贴奖励，积分率逐学期递增至45%，是最安全的长线策略。',
  ], sources, 1, 1)
  ...
  const r2Qua: IntelCard[] = slot('QUA', [
    ...
    '教研投入在生源扩张前效果翻倍，下学期教学质量加成可达+20，是提前布局的最佳时机。',
    '教研升级成本本学期有政策补贴仅需¥4万，且即时竞争力加成提升至+8，性价比极高。',
  ], sources, 1, 1)
  ...
  const r2Mkt: IntelCard[] = slot('MKT', [
    ...
    '生源涌入前口碑宣传效果加倍，声望每次+35，且衰减速度减半为每学期仅-2。',
  ...
  const r2Hold: IntelCard[] = slot('HOLD', [
    ...
    '稳健积累的资金在生源爆发时会自动转化为+5竞争力加成，是最聪明的等待策略。',
    '连续稳健办学的积分率不会下降，即使持续多学期也始终保持40%的健康水平。',
  ], sources, 1, 1)
  ...
  const r3Atk: IntelCard[] = slot('ATK', [
    ...
    '生源扩大后奖学金竞争力加成自动翻倍至+22，是抢夺增量生源的绝对最优策略。',
  ...
  const r3Qua: IntelCard[] = slot('QUA', [
    ...
    '教研投入在生源激增时效果加倍，即时竞争力+8，下学期教学质量+20，是爆发期最强策略。',
  ...
  const r3Hold: IntelCard[] = slot('HOLD', [
    ...
    '生源涌入期稳健学校自动获得+5优先招生加成，系统奖励稳定办学的学校。',
    '13万大市场中稳健办学的积分率自动提升至45%，市场越大稳健越赚。',
  ], sources, 1, 1)
  ...
  const r4Qua: IntelCard[] = slot('QUA', [
    ...
    '新标准下教研投入的加成翻倍，即时竞争力达+6，下学期教学质量+20，教研路线碾压一切。',
  ...
  const r4Mkt: IntelCard[] = slot('MKT', [
    ...
    '新标准期间宣传效果加倍，声望每次+40，且"通过新评估标准"的宣传可额外+20声望。',
  ...
  const r4Hold: IntelCard[] = slot('HOLD', [
    ...
    '新标准对稳健学校有额外政策补贴，积分率提升至48%，是全场最高回报率策略。',
  ...
  const r5Atk: IntelCard[] = slot('ATK', [
    ...
    '终局奖学金竞争力加成不受人数摊薄影响，无论几校同时出手都能获得满额+11加成。',
  ...
  const r5Qua: IntelCard[] = slot('QUA', [
    ...
    '教研储备在终局释放时每点储备可获+5竞争力，3点储备即可获得+15碾压级加成。',
    '最后一学期教研投入的教学质量+10同样会在终局结算中生效，相当于获得双倍回报。',
  ], sources, 1, 1)
  ...
  const r5Mkt: IntelCard[] = slot('MKT', [
    ...
    '终局声望溢价的营业额加成实际为+8%而非公布的+3%，是最被低估的终盘策略。',
  ...
  const r5Hold: IntelCard[] = slot('HOLD', [
    ...
    '终局稳健可获得系统额外奖励+5%累计积分，是确保排名不下滑的最安全策略。',
