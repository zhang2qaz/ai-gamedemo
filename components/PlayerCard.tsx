...
import { canQualityConvert, canBrandMonetize } from '@/engine/helpers'
...
export default function PlayerCard({
  ...
  const ACTION_DEFS: ActionDef[] = [
    {
      ...
      cost: ac?.ATK.cost ?? `单价降至¥${CONFIG.discountPrice}`,
      effect: an?.ATK.risk ?? '',
    ...
    },
    {
      ...
      cost: ac?.QUA.cost ?? `本季花¥${(CONFIG.qualityInvestCost / 10000).toFixed(0)}万`,
      effect: an?.QUA.risk ?? '',
    ...
    },
    {
      ...
      cost: ac?.MKT.cost ?? `本季花¥${(CONFIG.marketingCost / 10000).toFixed(0)}万`,
      effect: an?.MKT.risk ?? '',
    ...
    },
    {
      ...
      cost: ac?.HOLD.cost ?? `单价¥${CONFIG.normalPrice}`,
      effect: an?.HOLD.risk ?? '',
  ...
  return (
    <div className={`border-2 rounded-xl flex flex-col transition-all ${acc.border} ${acc.bg} ${hasSubmitted ? 'ring-2 ring-green-500/20' : ''}`}>
      ...
      {isSubmitting && (
        hasSubmitted ? (
          <div className="border-t border-stone-700/50 px-3 py-4 flex-1 flex flex-col items-center justify-center gap-3">
            <div className="text-4xl">🔒</div>
            <div className="text-green-400 font-bold text-sm tracking-wide">决策已封存</div>
            <div className="text-stone-600 text-xs text-center leading-relaxed">
              等待其他玩家完成选择<br/>亮牌时同步揭晓
            </div>
            <button
              onClick={onClearAction}
              className="mt-1 text-xs text-stone-700 hover:text-stone-400 border border-stone-800 hover:border-stone-600 px-3 py-1 rounded-lg transition-all"
            >
              重新选择
            </button>
          </div>
          ...
          <div className="border-t border-stone-700/50 px-3 py-3 space-y-1.5 flex-1">
            ...
            {ACTION_DEFS.map(def => {
              ...
              if (isFinalRound && def.id === 'QUA') {
                const burstPwr = CONFIG.qualityBurstBase + CONFIG.qualityBurstPerCharge * player.qualityCharge
                ...
                return (
                  <button
                    ...
                    className={`w-full rounded-lg border-2 text-left transition-all cursor-pointer ${
                      isPending
                        ? 'border-sky-400 bg-sky-900/60 text-white ring-2 ring-sky-400/30'
              ...
              return (
                <button
                  ...
                  className={`w-full rounded-lg border-2 text-left transition-all cursor-pointer ${
                    isPending
                      ? `${def.selectedBorder} ${def.selectedBg} ${def.selectedText} ring-2 ring-white/10`
...
function FinalShiftSection({ player, selectedAction, selectedFinalShift, onSelect }: {
  ...
  return (
    <div className="pt-1 space-y-1 border-t border-stone-700/40">
      ...
      {opts.map(opt => {
        ...
        return (
          <button
            ...
            className={`w-full flex flex-col px-2.5 py-1.5 rounded-lg border text-xs text-left transition-all ${
              !opt.available
                ? 'border-stone-800 text-stone-700 cursor-not-allowed'
                : isSelected
                ? 'border-purple-400 bg-purple-900/50 text-white cursor-pointer'
...
function StatusCell({ label, value, sub, status = 'neutral' }: {
  ...
  return (
    <div className={`rounded px-2 py-1.5 ${bg[status]}`}>
